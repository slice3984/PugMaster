...
const command: Command = {
    ...
    exec: async (bot, message, params, defaults) => {
        ...
        if (params[0]) {
            ...
            if (!pickup) {
                return Util.send(message, 'error', 'no pickup found for the provided id');
            }
        } else {
            pickup = await StatsModel.getPickup(BigInt(message.guild.id));

            if (!pickup) {
                return Util.send(message, 'warn', 'there is no pickup record for this server');
            }
        }
        ...
        if (pickupSettings.playerCount === 2) {
            return Util.send(message, 'warn', 'pickup has to be a pickup with more than two players');
        }
